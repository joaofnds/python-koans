...
class AboutSets(Koan):
    ...
    def test_we_can_query_set_membership(self):
        self.assertTrue(127 in {127, 0, 0, 1} )
        self.assertTrue('cow' not in set('apocalypse now') )

    def test_we_can_compare_subsets(self):
        self.assertTrue(set('cake') <= set('cherry cake'))
        self.assertTrue(set('cake').issubset(set('cherry cake')) )

        self.assertFalse(set('cake') > set('pie'))
